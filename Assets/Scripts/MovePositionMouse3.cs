...
public class MovePositionMouse3 : MonoBehaviour
{
    ...
    void OnMouseDown()
    {
        ...
        upperRestriction = (levelManager.GetSpaceData(GetID(permitedPosition[1]))
            ...
            middlePos + (levelManager.scale / 2) - 5.5f : (isXconstant) ? levelManager.getBoundsFor("U") - levelManager.scale / 2 : levelManager.getBoundsFor("Ru") + levelManager.scale/2;
        ...
        lowerRestriction = (levelManager.GetSpaceData(GetID(permitedPosition[2])) != -1 && levelManager.GetSpaceData(GetID(permitedPosition[2]))
            ...
            != blockID) ? middlePos - 4.5f : (isXconstant) ? levelManager.getBoundsFor("B") + levelManager.scale / 2 : (blockID == 0) ? levelManager.getBoundsFor("L") -50f : levelManager.getBoundsFor("L") /2;
    ...
    public void OnMouseDrag()
    {
        ...
        if(blockID == 0)
        {
            levelManager.setDidWin(transform.position.x <= levelManager.getBoundsFor("L") -20f);
        }
    }
